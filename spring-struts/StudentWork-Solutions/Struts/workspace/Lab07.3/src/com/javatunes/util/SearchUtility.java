...
public class SearchUtility {
   ...
   // searches catalog by maximum price
   public static Collection<MusicItem> findByMaxPrice(float price) {
      // create a bucket for results
      Collection<MusicItem> result = new ArrayList<MusicItem>();

      // iterate through our catalog, looking for a match
      for (MusicItem item : c_catalog) {
         if (item.getPrice().floatValue() <= price)  {
            result.add(item);
         }
      }
      return result;
   ...
   public static Collection<MusicItem> getCDCatalog() {
       return Collections.unmodifiableCollection(c_catalog);
   }    
