...
(
   ...
   CONSTRAINT  PK_ItemBandMembers  PRIMARY KEY(ITEM_ID, Member),
   CONSTRAINT  FK_BM_to_Item    FOREIGN KEY(ITEM_ID) REFERENCES Item(ITEM_ID)
);
...
(
   ...
   CONSTRAINT  PK_Downloadable  PRIMARY KEY(ITEM_ID),
   CONSTRAINT  FK_DI_to_Item    FOREIGN KEY(ITEM_ID) REFERENCES Item(ITEM_ID)
);


CREATE TABLE Inventory
(
   INV_ID      BIGINT NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
   ITEM_ID     BIGINT NOT NULL,
   Location    VARCHAR(25) NOT NULL,
   Quantity    INTEGER       NOT NULL,
   Version     INTEGER,
   CONSTRAINT  PK_Inventory        PRIMARY KEY(INV_ID),
   CONSTRAINT  UNQ_Item_Location   UNIQUE(ITEM_ID, Location),
   CONSTRAINT  FK_IV_to_Item       FOREIGN KEY(ITEM_ID) REFERENCES Item(ITEM_ID)
);

CREATE TABLE PurchaseOrder
(
   PO_ID       BIGINT            NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),
...
(
   ...
   CONSTRAINT       PK_PurchaseItem      PRIMARY KEY(PurchaseOrderID, LineNumber),
   CONSTRAINT       FK_to_PO  FOREIGN KEY(PurchaseOrderID) REFERENCES PurchaseOrder(PO_ID)
);
